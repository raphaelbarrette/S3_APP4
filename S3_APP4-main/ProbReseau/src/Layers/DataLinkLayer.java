...
public class DataLinkLayer extends Layer{
    ...
    protected void getFromUp(byte[] PDU) {
        ...
        packetEnvoyer++;
        logToFile("Paquet envoyé. Numéro de paquet: " + packetEnvoyer);
    ...
    protected void getFromDown(byte[] PDU) throws TransmissionErrorException, IOException {
        ...
        if (ValeurCRC != oldCRC) {
            ErreurCRC++;
            logToFile("Erreur CRC détectée. Nombre total d'erreurs CRC: " + ErreurCRC);
            return;
        }

        packetRecu++;
        logToFile("Paquet reçu. Numéro de paquet: " + packetRecu);
        sendUp(paquet);
    }
